...
public class User {
    ...
    public static int convertToLevel(int xp) {
        ...
        int level = 1;
        while (xp > 0) {
            xp -= Math.floor(100 * Math.pow(2, 0.05 * (level - 1)));
            if (xp >= 0)
                level++;
        }
    ...
    public static int toNextLevel(int currentLevel) {
        return (int) Math.floor(100 * Math.pow(2, 0.05 * (currentLevel - 1)));
    }
    ...
    public static class UserBuilder {
        ...
        public User build(String... args) {
            ...
            user.maxHp = calculateMaxHP(convertToLevel(this.xp), user.baseConst + user.constMod);
            Log.v("MAXHP", calculateMaxHP(1, 4) + "");
            user.maxMana = calculateMaxMana(convertToLevel(this.xp), user.baseIntel + user.intelMod);

            user.hp = (this.hp > user.maxHp || this.hp < 0) ? user.maxHp : this.hp;
